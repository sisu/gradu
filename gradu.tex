...
We show that the algorithm can be generalized to higher dimensions, leading to an algorithm that finds the minimum link path in $D$-dimensional domain in $O(n^{D-1}\log^{D-1} n)$ time and $O(n^{D-1})$ space.
\end{abstract}
...
For example we can create a segment tree that supports adding intervals, and finding the maximal number of overlapping any point in a query interval in $O(\log n)$ time.
