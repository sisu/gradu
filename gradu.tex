...
\renewcommand{\algorithmicensure}{\textbf{Effect:}}
\algnewcommand\algorithmicoutput{\textbf{Output:}}
\algnewcommand\Output{\item[\algorithmicoutput]}
\algnewcommand\LComment[1]{\(\triangleright\) #1}
...
\newcommand\fspaceb{\ensuremath{\mathcal{B}}\xspace}
...
\newcommand\canonpar[1]{\ensuremath{\textsc{a}(#1)}\xspace}
\newcommand\canonpari[2]{\ensuremath{\textsc{a}_{#2}(#1)}\xspace}
...
\newcommand\cleardt{\ensuremath{\texttt{ClearRectangle}}\xspace}
...
\newcommand\pop[1]{\ensuremath{\texttt{pop}(#1)}\xspace}
...
\newcommand\decompa[2]{\ensuremath{\textsc{dec}_{#1}(#2)}\xspace}
...
\newcommand\cellE{\ensuremath{\mathtt{CellEvent}}\xspace}
\newcommand\cellEs{\ensuremath{\mathtt{CellEvent}}s\xspace}
\newcommand\obsE{\ensuremath{\mathtt{ObstacleEvent}}\xspace}
\newcommand\obsEs{\ensuremath{\mathtt{ObstacleEvent}}s\xspace}
\newcommand\addE{\ensuremath{\mathtt{AddRectangleEvent}}\xspace}
\newcommand\addEs{\ensuremath{\mathtt{AddRectangleEvent}}s\xspace}
\newcommand\cellof[1]{\ensuremath{\mathtt{cell}(#1)}\xspace}
\newcommand\rectof[1]{\ensuremath{\mathtt{rect}(#1)}\xspace}
\newcommand\hrectof[1]{\ensuremath{\mathtt{rect}(#1)}\xspace}
...
\numberofpagesinformation{\numberofpages\ pages + \numberofappendixpages\ appendix}

\classification{\protect{\ \\
Theory of computation $\rightarrow$ Design and analysis of algorithms $\rightarrow$ Graph algorithms analysis $\rightarrow$ Shortest paths\\
Theory of computation $\rightarrow$ Randomness, geometry and discrete structures $\rightarrow$ Computational geometry\\
}}
...
The goal is to find a path that does the minimum number of turns amidst obstacles in a continuous space.
We focus on the most classical variant, the \emph{rectilinear} minimum link path problem, where the path and the obstacles are restricted to the directions of the coordinate axes.

We study the rectilinear minimum link path problem in the plane and in the three-dimensional space, as well as in higher dimensional domains.
We present several new algorithms for solving the problem in domains of varying dimension.
For the planar case we develop a simple method that has the optimal $O(n\log n)$ time complexity.
For three-dimensional domains we present a new algorithm with running time $O(n^2\log^2 n)$, which is an improvement over the best previously known result $O(n^{2.5}\log n)$.
The algorithm can also be generalized to higher dimensions, leading to an $O(n^{D-1}\log^{D-1} n)$ time algorithm in $D$-dimensional domains.

We describe the new algorithms as well as the data structures used.
The algorithms work by maintaining a reachable region that is gradually expanded to form a shortest path map from the starting point.
The algorithms rely on several efficient data structures:
the reachable region is tracked by using a simple recursive space decomposition, and the region is expanded by a sweep plane method that uses a multidimensional segment tree.
\end{abstract}
...
Various kinds of path finding problems are a common topic in algorithmics~\cite{survey}.
...
The typical goal is to minimize the Euclidean length of the path, but for some applications other distance metrics are more relevant.
For example a robot might move quickly in a straight line but take a lot of time to turn, so a long but simple route can be preferable to a shorter but more complex path.
...
We start by defining $\reach{0}=\set{\spt}$, and iteratively compute \reach{k+1} based on \reach{k}.
The iteration continues until we find $k$ such that $\ept\in\reach{k}$.
This iteration is often called the \emph{staged illumination}, because the computation of \reach{k+1} can be illustrated as placing light sources on the boundaries of \reach{k} and observing the region that gets lit.
...
Each rectangle $a$ in the decomposition is defined by its $x$-range \xrange{a} and the $y$-range \yrange{a}, which we assume to be half-open intervals.
...
When a rectangle $a$ is added to the tree, \yranget{a}{end} is initially left undefined because we don't know how tall the rectangle will be until $a$ is removed from the tree.
...
\begin{algorithm}
\caption{Decompose the free space into \decomp{x}.}\label{alg:split2d}
\begin{algorithmic}
\Require Set of obstacle line segments $E$.
\Output Decomposition \decomp{x} of the region defined by $E$.
\State $T\gets \text{Empty binary search tree}$.
\State Sort $E$ by the $y$ coordinates of the edges.
\State $R\gets$ Empty list of cells.
\ForAll{$e\in E$}
	\If{Normal of $e$ points down}
		\State \LComment Exactly one $x$-interval in $T$ intersects \xrange{e}.
...
A cuboid is a three-dimensional hyperrectangle.
...
The number of overlapping rectangles in \decompa{x}{A} and \decompa{x}{B} is $O(nk)$.
\end{lem}
\begin{proof}
...
\ForAll{$e\in E$}
	...
	\If{$e$ is a bottom edge of $r$}
		\ForAll{$u\in$ elements in $T_t$ intersecting $e$}
...
We use the terms rectangle and hyperrectangle interchangeably when it is obvious that we are referring to multidimensional rectangles.
...
\State Sort $Z$ in increasing order.
\State $R\gets$ Empty list of result cells.
\State $M\gets$ Empty binary search tree mapping $(D-1)$-dimensional rectangles to indices \\
	\hspace{10mm} in $R$.
\ForAll{$z\in Z$}
...
The number of pairs of overlapping $D$-dimensional rectangles in the decompositions of $A$ and $B$ is $O(n^{D-1}k)$.
\end{lem}
\begin{proof}
Proof by induction on $D$.
...
\begin{algorithm}
\caption{Find all the overlapping pairs of two sets of $D$-dimensional rectangles.}
\label{alg:overlapdd}
\begin{algorithmic}
\Require Sets of disjoint $D$-dimensional rectangles $A$ and $B$.
\Output All the overlapping pairs of elements in $A$ and $B$.
\Procedure{\findol}{$D,A,B$}
	\If{$D=2$}
		\State \Return overlapping pairs computed with Algorithm~\ref{alg:overlap2d}.
	\EndIf
	\State $E\gets$ List of all the rectangles in $A$ and $B$ perpendicular to $D$-axis.
	...
	\ForAll{$g\in G$}
		...
		\State $S_A\gets$ Elements of $A$ with $D$-range lower bound $z$.
		\State $S_B\gets$ Elements of $B$ with $D$-range lower bound $z$.
		\State $T_A\gets$ Elements of $A$ with $D$-range upper bound $z$.
		\State $T_B\gets$ Elements of $B$ with $D$-range upper bound $z$.
...
For each unit we recursively search for new links between the newly added rectangles, as well as between old and new rectangles.
We maintain the set of rectangles currently intersecting the sweep hyperplane to avoid adding duplicates to the result.
...
In this thesis we define \inter{s} to be a half-open real number interval for each $s$.
...
\begin{algorithmic}
\Require Interval $I$.
\Ensure Calls $\visitc$ and $\visitp$ for all the elements in \canon{I} and in \canonpar{I} respectively.
\Procedure{\iterc}{$s$}
	\Comment{Visit nodes in the subtree rooted at $s$}
...
\item Recompute \nodeps{s} to match the remaining intervals for all nodes $s\in\canonpar{I}$.
\end{enumerate}
...
\Ensure Sets \hasrect{s} for each canonical node $s$ of $R$.
\Procedure{\adddt}{$D,s$}
...
\begin{algorithmic}
\Require $D$-dimensional rectangle $R$.
\Output Whether \hasrect{s} is true for any node $s$ matching $R$.
...
\begin{lem}\label{lem:rectaddtime}
\adddt and \checkdt both have time complexity $O(\log^D\segsize)$.\end{lem}
\begin{proof}
Proof by induction on $D$.
...
For each axis $i$ where \rect{s} is not minimal, $s$ has links to two child nodes that divide \rect{s} into two parts along a hyperplane perpendicular to the $i$-axis.
Thus each node has between 0 and $2D$ links to child nodes.
The children splitting the node $s$ along axis $i$ are denoted \leftu{s}{i} and \rightu{s}{i}.
...
\begin{figure}\centering
	\includegraphics[width=0.5\textwidth,page=2]{fig/uremove}
...
This mask is used to determine which bits of \hasbita{s} to access when adding a rectangle or querying the tree.
...
\Ensure Sets \hasbits{s}{b} for each node $s$ and bitmask $b$ matching $R$.
\Procedure{\adddt}{$d,s$}
...
\begin{algorithmic}
\Require $D$-dimensional hyperrectangle $R$.
\Output Whether \hasbits{s}{b} is set for any node $s$ and bitmask $b$ matching $R$.
\Procedure{\checkdt}{$d,s$}
\Comment{Check subtree rooted at node $s$}
	\If{$d=0$}
...
\begin{lem}\label{lem:uaddtime}Algorithm~\ref{alg:uadd} and Algorithm~\ref{alg:ucheck} both have time complexity $O(\log^D\segsize)$.\end{lem}
\begin{proof}
The proof is similar to Lemma~\ref{lem:rectaddtime}.
The algorithms use recursion with argument $d$ defining the axis along which the tree is traversed.
Initially the recursive functions are called with argument $d=D$.
We prove the claim by induction on $d$.

For the base case $d=0$ both algorithms finish in constant time.
For $d\ge 1$, the algorithms iterate over the canonical nodes and their ancestors by moving along axis $d$, making $O(\log\segsize)$ calls with argument $d-1$.
By induction the operations with argument $d-1$ are done in $O(\log^{d-1}\segsize)$ time, so the time complexity is $O(\log^d\segsize)$.
\end{proof}
...
\Ensure Copies \hasbita{t} to \hasbita{\leftu{t}{a}} and to \hasbita{\rightu{t}{a}} for all nodes $t$ that can be accessed from $s$ by moving along axes $1\dots d$.
...
\Ensure Generates \hasbita{t} based on the descendants of $t$ for all nodes $t$ that can be accessed from $s$ by moving along axes $1\dots d$.
...
\Procedure{\recompn}{$s$}
	...
	\ForAll{$a \in 1\dots D$}
		...
		\EndIf
		\State \LComment $\rect{s}_a$ is not minimal, so \leftu{s}{a} and \rightu{s}{a} are defined.
...
The size of \canonpari{R_d}{d} is $O(\log\segsize)$, so by induction the time complexity is $O((\log\segsize)\segsize^{d-1} + \segsize(\segsize^{d-2}\log\segsize) + k)=O(\segsize^{d-1}\log\segsize + k)$.
\end{proof}
...
For even $k\ge 2$, the region \reach{k+1} consists of the rectangles $r\in\decomp{x}$ that intersect \reach{k}.
For odd $k\ge 3$ the same applies for \decomp{y}.
\end{lem}
\begin{proof}
Let $k\ge 2$ be even.
The region \reach{k+1} is formed by illuminating in horizontal direction from \reach{k}.
...
Let $k$ be an even number and $r\in\decomp{y}$ contained in \reach{k}.
...
We are implicitly searching through the intersection graph of \decomp{x} and \decomp{y}, but there is no need to actually compute the vertical decomposition.
...
\item For odd $k$, \reach{k} consists of a set of rectangles $R_k\subseteq\decomp{x}$ when $k\ge 3$.
	...
	We inspect which rectangles can be reached by a vertical line starting from any point in \reach{k}, and form $\hat{R}_{k+1}=\set{r \mid r\in\decomp{x}, r\cap\reach{k+1}\neq\emptyset}$.
...
If a rectangle $r$ is discovered on step $k$, then it is fully illuminated in \reach{k+1}, and each point visible from $r$ is illuminated in \reach{k+2}.
...
Next we show how to implement illumination in $y$-direction starting from \reach{k}, which is represented as a set of rectangles $R\subseteq\decomp{x}$.
...
In the pseudocode we mark the step when the illumination first reaches each rectangle $r$ as \stepof{r}.
...
\State Form \decomp{x} from $O$ with Algorithm~\ref{alg:split2d}.
\State $R\gets\set{r\in\decomp{x} \mid \spt\in r}$
\If{\ept inside the only rectangle of $R$}
	\State \Return 1 if $\spt_y=\ept_y$, 2 otherwise.
\EndIf
\State $k\gets 1$
\While{$R\neq\emptyset$ and \ept not inside any rectangle of $R$}
	\ForAll{$r\in R$}
	...
	\State $H_{-y}\gets H$ illuminated in direction $-y$ by Algorithm~\ref{alg:light2d}.
	\If{$\ept$ found during any of the sweeps}
		\State \Return $k+1$.
	\EndIf
...
The path is computed using the staged illumination paradigm, and on each step we construct \reach{k+1} from \reach{k} by applying a sweep plane algorithm in all the coordinate axis directions.
...
The decomposition is used to implement the plane sweep algorithm for computing \reach{k+1} from \reach{k}.
...
We use a sweep plane algorithm to compute \reach{k+1} based on \reach{k}~\cite{restricted}.
...
Note that while the region \reach{k} plays a central role in the design of the algorithm, we don't explicitly maintain this region.
...
\caption{Illuminate by a plane sweep in direction $+z$ starting from provided events.
	Produces \addEs and \cellEs in directions $\pm x$ and $\pm y$ for the next illumination step.}\label{alg:sweep3d}
\begin{algorithmic}
\Require Initial event set $E$.
\Output List of \addEs and \cellEs for the next illumination step.
...
\While{$Q$ is not empty}
	...
	\ElsIf{$e$ is \cellE}
		\If{\rectof{e} touches any rectangle in $T$}
			...
			\State Insert \cellEs for $c$ in directions $\pm x$ and $\pm y$ into $R$.
	...
	\ElsIf{$e$ is \obsE}
		...
		\State Generate \addEs for the cleared canonical nodes into $R$ \\
...
We can find the minimum link path between two points by stopping the illumination when it reaches the endpoint and tracing back the path.
The stopping condition and path generation have been omitted to focus on the core algorithm.

\begin{algorithm}
\caption{Run staged illumination in a three-dimensional domain.}\label{alg:minlink3d}
\begin{algorithmic}
\Require Set of obstacle faces and a starting point \spt.
\Ensure Discovers all the points reachable from \spt with the staged illumination.
...
As a first step in the event generation, we create an \addE on each side of every cleared canonical node.

Each generated event $e$ is assigned a rectangle \rectof{e} that will be added to the unified tree when the event is processed.
Each rectangle is defined by two intervals; for example if $e$ is an event in direction $+x$ then \rectof{e} is defined by a $z$-range and a $y$-range.
The $z$-range is bounded by the $z$-coordinate of the \obsE generating $e$ and the $z$-coordinate of the \addE that added the cleared canonical node.
The other range is defined by the side of the cleared canonical node.

Creating \addEs for each cleared canonical node can lead to a large number of events, because each rectangle is split into $O(\log^2 n)$ canonical nodes in the tree.
...
The two-dimensional illumination illuminates only maximal rectangles of \decomp{x} and \decomp{y}, producing a total of $O(n)$ illuminated rectangles according to Theorem~\ref{theo:minlink2d}.
...
On each \cellE we perform a single lookup to the unified tree and iterate over the neighbors of the cell.
By Lemma~\ref{lem:uaddtime} the lookup can be done in $O(\log^2 n)$ time.
Since the decomposition has $O(n^2)$ cells and $O(n^2)$ links, the total time taken in processing \cellEs is $O(n^2\log^2 n)$.

On each \addE we insert a new rectangle to the unified tree in $O(\log^2 n)$ time by Lemma~\ref{lem:uaddtime}.
...
The sweep plane is replaced by a $(D-1)$-dimensional hyperplane, and each event $e$ defines a $(D-1)$-dimensional hyperrectangle \hrectof{e}.
...
\caption{Illuminate by a hyperplane sweep in direction $+D$ starting from provided events.
	Produces \addEs and \cellEs in directions other than $\pm D$ for the next illumination step.}
\label{alg:sweepdd}
\begin{algorithmic}
\Require Initial event set $E$.
...
\While{$Q$ is not empty}
	...
	\ElsIf{$e$ is \cellE}
		\If{\hrectof{e} touches any hyperrectangle in $T$}
			\State $c\gets\cellof{e}$.
			\State Insert \cellEs for $c$ in directions other than $\pm D$ into $R$.
...
\label{alg:minlinkdd}
\begin{algorithmic}
\Require Set of obstacle faces and a point \spt.
\Ensure Discovers all the points reachable from \spt with the staged illumination.
...
On each \cellE we perform a single lookup to the unified tree and iterate over the neighbors of the cell.
By Lemma~\ref{lem:uaddtime} the lookup can be done in $O(\log^{D-1} n)$ time.
...
On each \addE we insert a new rectangle to the unified tree in $O(\log^{D-1} n)$ time by Lemma~\ref{lem:uaddtime}.
