...
\newcommand\recomp{\ensuremath{\textsc{ComputeSubtreeData}}\xspace}
...
Figure~\ref{fig:uremove} shows the steps of the clearing process.
...
\Procedure{ComputeSubtreeData}{$d,s$}
	\If{$d=0$}
		\State $\textsc{ComputeSubtreeDataForNode}(s)$.
		\State Return.
	\ElsIf{$\rect{s}_d\cap R_d=\emptyset$}
		\State Return.
	\EndIf
...
Run staged illumination starting from \spt within :ree space \fspace such that the first link is horizontal.
\begin{algorithmic}
...
The path can then be constructed similar to regular shortest path computation in a graph~\cite{clrs}:
...
Wagner~\cite{unified} suggest that it might be possible to speed up the operations on the unified segment tree by a logarithmic factor through the use of fractional cascading.
