...
This approach works also for 3D, but since there are $O(n^2)$ cells and clearing a rectangle from the unified tree requires $O(n\log n)$ steps, generating \obsEs for each of the cells would increase the time complexity significantly.
...
\item During a sweep in direction $+z$ we hit the obstacle $o$.
	...
	When these sweeps hit an \obsE, we may then generate events in direction $+z$ on plane $P$.
\item During a sweep in direction $\pm x$ or $\pm y$ the obstacle face $o$ cuts a rectangle in the tree such that the remaining part has boundary on $P$.
	Then when the sweep processes another \obsE, we generate an \addE in direction $+z$ on plane $P$.
\item If on the previous step we created $\addE$ on plane $P$, then we also created events in directions $\pm x$ or $\pm y$ that have plane $P$ as one of the sides.
	From those events we again add rectangles with $P$ as one of the sides to the unified tree, leading to creation of \addEs on $P$ during an \obsE.
\end{enumerate}
