...
The algorithms work by maintaining a reachable region that is gradually expanded to form a shortest path path from the starting point.
Two data structures play a key role in the algorithms: a space decomposition and the segment tree.
The reachable region is tracked by using a simple recursive space decomposition.
The region is expanded by a sweep plane method that uses a multidimensional segment tree.
\end{abstract}
...
\subsection{Search in an intersection graph}
...
\EndFor
\While{$E_d$ is not empty for some $d$}\Comment $E_d$-sets define the boundary of $\reach{k}$.
	\ForAll{$d\in\set{\pm x,\pm y,\pm z}$}
