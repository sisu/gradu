...
\newcommand\addE{\ensuremath{\mathtt{AddRangeEvent}}\xspace}
\newcommand\addEs{\ensuremath{\mathtt{AddRangeEvent}}s\xspace}
...
We introduce the new algorithms as well as the data structures used.
The algorithms use the same basic principle as many other search algorithms: maintain a reachable region that is gradually expanded to form the shortest path map from the starting point.
The reachable region is tracked by using a simple recursive space decomposition.
The region is expanded by a sweep plane method that uses a multidimensional segment tree, which is a well known geometric data structure.
\end{abstract}
...
Following a typical convention, we assume that the domain is given as a list of $n$ rectangular obstacle faces~\cite{wagner}.
...
\item A new algorithm for the 3-dimensional minimum link path problem, improving the running time from the previous best result of $O(n^{2.5}\log n)$ to $O(n^2\log^2 n)$~\cite{restricted}.
...
The \emph{segment tree} is a generic data structure for working with intervals.
...
Finding the $z$-links in the 3-dimensional decomposition is equivalent to finding the set of overlapping pairs between $A$ and $B$.
...
Consider the sweep in direction $+z$.
...
\Ensure Generates \addEs for the next illumination step and marks newly found obstacles as visited.
...
\While{$Q$ is not empty}
	...
	\ElsIf{$e$ is \obsE}
		...
		\State Generate \addEs for the cleared canonical rectangles (Section~\ref{sec:evtgen}).
	\EndIf
\EndWhile
\end{algorithmic}
\end{algorithm}
...
\item During a sweep in direction $+z$ we hit the obstacle $o$.
	...
	During the sweeps of the following step these events cause us to add rectangles to the unified tree.
...
After the illumination has reached the obstacle $o$, the illumination on $P$ proceeds exactly like the two-dimensional staged illumination in the cross section of $P$.
The two-dimensional illumination illuminates only maximal rectangles of \decomp{x} and \decomp{y}, producing a total of $O(n)$ illuminated rectangles.
...
Thus the event count is within a constant factor of the event count of the two-dimensional staged illumination.
...
Algorithm~\ref{alg:minlinkdd} implements the $D$-dimensional staged illumination by using the hyperplane sweep in all directions on each step.
...
\begin{lem}\label{lem:added}The number of \addEs generated during the illumination after after the filtering is $O(n^{D-1})$.\end{lem}
\begin{proof}
...
We classify the events remaining after the filtering similarly to the 3D case:
\begin{enumerate}
...
We presented new algorithms for the problem with lower asymptotic running times than the previous known solutions.
