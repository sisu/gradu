...
\begin{figure}\centering
	...
	\end{subfigure}
	\caption{A 3-dimensional domain is decomposed into cuboids by taking the horizontal decomposition of each cross section. The highlighted cell is shared between the two cross sections, so the cuboids on the two levels are merged.}\label{fig:split3d}
\end{figure}
...
\Output 3-dimensional decomposition of the volume defined by $E$.
...
\Output All overlapping pairs of elements in $A$ and $B$
...
Similarly for $D$-dimensional trees we define product $c_1\times c_2\times\dots\times c_D$ to be the node of a $D$-dimensional segment tree defined by positions of 1-dimensional tree nodes.
...
\begin{algorithmic}
\Require $D$-dimensional hyperrectangle $R$
\Ensure Modifies \hasbits{}{} accordingly.
