...
\begin{algorithmic}
\Require Interval $I$
\Ensure Call $\visitc$ and $\visitp$ for all the elements in \canon{I} and in \canonpar{I} respectively.
\Procedure{\iterc}{$s$}
...
\begin{algorithmic}
\Require Interval $I$
\Ensure Clear $I$ from the segment tree.
\Procedure{\cleariv}{$s$}
...
\Ensure Set \hasrect{s} for each canonical node $s$ of $R$.
\Procedure{\adddt}{$D,s$}
...
\Ensure Set \hasbits{s}{b} for each node $s$ and bitmask $b$ matching $R$
\Procedure{\adddt}{$d,s$}
