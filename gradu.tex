...
\Ensure Decomposition \decomp{x} of the region defined by $E$
...
\Ensure 3-dimensional decomposition of the volume defined by $E$
...
\begin{algorithmic}
\Require $D$-dimensional rectangle $R$
\Ensure Whether \hasrect{s} is true for any node $s$ matching $R$.
...
\begin{algorithmic}
\Require $D$-dimensional hyperrectangle $R$
\Ensure Whether \hasbits{s}{b} is set for any node $s$ and bitmask $b$ matching $R$
\Procedure{\checkdt}{$d,s$}
\Comment{Check subtree rooted at node $s$}
	\State $R\gets\texttt{false}$.
	\If{$d=0$}
...
\begin{algorithm}
\caption{Illuminate by plane sweep in direction $+z$ starting from a provided event set $E$.}\label{alg:sweep3d}
\begin{algorithmic}
