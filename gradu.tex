...
This allows us to generalize Lemma~\ref{lem:segintersect} for segment interaction to work for $D$-dimensional hyperrectangles.
...
To find pairs of hyperrectangles fulfilling the conditions of Lemma~\ref{lem:rectintersect}, we store \emph{two} $D-1$ dimensional inner trees in every outer node of a $D$-dimensional segment tree:
\begin{description}
\item[\subtree{s}] where we store hyperrectangles $R$ for which $s\in\canoni{R_D}{D}$,
\item[\subtreep{s}] where we store hyperrectangles $R$ for which $s\in\canonpari{R_D}{D}$.
\end{description}

We can implement hyperrectangle insertion and query operations by using these fields.
...
Add hyperrectangle $R$ to a $D$-dimensional segment tree.
\begin{algorithmic}
...
Query whether hyperrectangle $R$ intersects with any rectangle in the $D$-dimensional segment tree.
\begin{algorithmic}
...
Suppose we first add hyperrectangle $A$ and then query for hyperrectangle $B$ in the tree.
The \checkdt operation returns true if the two operations end up in any common dimension~0 node.
The only way the two operations end up in a common node is if in each dimension $d$ we are in a canonical node of at least one of $A_d$ and $B_d$, and in either a canonical node or in an ancestor of a canonical node for the other hyperrectangle.
...
However clearing a hyperrectangle in this structure is a more complex operation.
...
In the unified tree there is no concept of outer and inner nodes, and each node represents a $D$-dimensional hyperrectangle.
...
We consider again checking whether the query hyperrectangle intersects with any of the hyperrectangles in the tree.
