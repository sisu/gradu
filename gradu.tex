...
\begin{algorithmic}
\Require Node $s$ of the segment tree
\Ensure Clear interval $I$ from subtree $s$
\Procedure{\cleariv}{$s$}
...
\begin{algorithm}
\caption{Add rectangle $R$ to a $D$-dimensional segment tree.}\label{alg:rectadd}
\begin{algorithmic}
\Require Node $s$ of the $D$-dimensional segment tree
\Ensure \dots
\Procedure{\adddt}{$D,s$}
...
\begin{algorithm}
\caption{Query whether rectangle $R$ intersects with any rectangle in the $D$-dimensional segment tree.}\label{alg:rectcheck}
\begin{algorithmic}
