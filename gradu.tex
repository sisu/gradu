...
Prosessi muistuttaa läheisesti leveyssuuntaista hakua, mutta oleellisena erona hakua ei tehdä verkossa, vaan jatkuvassa avaruudessa.
...
Solmun $s$ lapsisolmut $\leftc{s}$ ja $\rightc{s}$ jakavat välin $\inter{s}$ kahteen pienempään osaan: $\inter{s}=\inter{\leftc{s}}\cup\inter{\rightc{s}}$, $\inter{\leftc{s}}\cap\inter{\rightc{s}}=\emptyset$.
...
\begin{lem}\label{lem:kanonlog}Kaikille väleille $v$ pätee $\size{\canon{v}}=O(\log\segsize)$\end{lem}
\begin{proof}
...
Siis algoritmin suoritusaika on verrannollinen kanonisten solmujen ja niiden esivanhempien määrään, joka on lemmojen~\ref{lem:kanonlog} ja~\ref{lem:kanonplog} nojalla $O(\log\segsize)$.
...
Jokaiselle $u$-särmiölle $s$ pätee
...
Lemman~\ref{lem:canond} ja lemman~\ref{lem:kanonlog} yhdistelmällä voidaan helposti rajata kanonisen solmujoukon koko.

\begin{kor}Jokaiselle $u$-särmiölle $s$ pätee $\size{\canon{s}}=O(\log^u\segsize$).\end{kor}
\begin{proof}
$$
...
Lemman~\ref{lem:canond} avulla minkä tahansa $u$-särmiön kanoninen joukko voidaan laskea soveltamalla algoritmia~\ref{alg:seghaku} jokaisen koordinaattiakselin suunnassa erikseen;
...
Yksiulotteisessa segmenttipuussa solmun $s$ esivanhemmat ovat tasan ne solmut $t$, joille pätee $\inter{s}\subsetneq\inter{t}$.
Moniulotteisessa tapauksessa osa solmuista voi peittää kokonaan solmun $s$ olematta kuitenkaan $s$:n vanhempi puussa, kuten selviää kuvasta~TODO.
Määrittelemmä \emph{laajennetun esivanhempien joukon} $\epar{s}$, joka sisältää kaikki solmut $t$ joille $\inter{s}\subsetneq\inter{t}$.
Laajennettu esivanhempien joukko voidaan laskea kanonisen solmujoukon tavoin ulottuvuus kerrallaan, kuten selviää seuraavasta lemmasa.

\begin{lem}
Merkitään $I(S) = \set{\inter{x} \mid x\in S}$.
Jokaiselle $u$-ulotteisen segmenttipuun solmulle $s$ pätee
...
\begin{lem}Jos $a\cap b\neq\emptyset$, niin vähintään yksi seuraavista pätee:
\begin{itemize}
...
Jos kahden peräkkäisen $xy$-tason hajotelma sisältää yhteisen suorakaiteen, yhdistetään niitä muodostetut särmiöt.
Seuraava algoritmi selventää tätä ideaa.

\begin{alg}\label{alg:jako3d}
Jaa estejoukon $E$ rajoittama alue suorakulmaisiin särmiöihin.
\begin{algorithmic}
\State $Z\gets \text{kaikkien esteiden $z$-koordinaatit}$.
\State Järjestä $Z$ kasvavaan järjestykseen.
...
\begin{lem}Algoritmi~\ref{alg:jako3d} jakaa alueen $O(n^2)$ suorakulmaiseen särmiöön.\end{lem}
\begin{proof}HT\end{proof}

Laajennetaan algoritmia~\ref{alg:jako3d} niin, että se laskee myös linkit muodostettujen solujen välillä.

\begin{lem}Algoritmin~\ref{alg:jako3d} tuottaman hajotelman solujen välillä on $O(n^2)$ linkkiä.\end{lem}
\begin{proof}HT\end{proof}
