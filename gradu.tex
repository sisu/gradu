...
The goal is to find a path amidst obstacles in a plane that does a minimal number of turns.
We focus on the rectilinear variant of the problem, where the path and the obstacles can only use the directions of the coordinate axes.

We study the rectilinear minimum link path problem in plane and in 3D dimensional space, as well as in higher dimensional domains.
We develop new algorithms for the 3D and the high-dimensional case that achieve better asymptotic running time than the previous known methods.
For the 3D rectilinear path problem we present an algorithm with running time $O(n^2\log^2 n)$ and space complexity $O(n^2)$ in a domain with $n$ obstacle edges.
We show that the algorithm can be generalized to higher dimensions, leading to an algorithm that finds the minimum link path in $D$-dimensional domain in time $O(n^{D-1}\log^{D-1} n)$ and space $O(n^{D-1})$.
\end{abstract}
...
Finally, we extend the solution to the 3D rectilinear problem to work in $D$-dimensional domains, giving an algorithm with running time $O(n^{D-1}\log^{D-1}n)$.
...
\item Extending the unified segment tree to higher than 2 dimensions and showing how clearing a hyperrectangle can be implemented. The original paper by Wagner~\cite{unified} mentions the possibility of extending the structure to higher dimensions, but does not provide any details.
\end{itemize}
...
However the number of links in a BSP-based decomposition can be as high as $O(n^{2.5})$, so we opt for a higher number of cells to reduce the number of links.
...
Figure~\ref{fig:split3d} displays how the 3D decomposition is created using a series of 2D decompositions.
...
\begin{proof}
The hyperrectangles can be constructed in time $O(n^{D-1}\log n)$ according to Lemma~\ref{lem:splitddtime}.
The total number of links between the nodes is $k=O(n^{D-1})$ according to Lemma~\ref{lem:splitddcount}.
Computing the links between all the layers can then be done in time $O(n^{D-1}\log n+k)=O(n^{D-1}\log n)$ by Lemma~\ref{lem:overlapddtime}.
...
\item Completely clear each each subtree rooted in any node $s\in\canon{I}$.
...
After the child nodes are cleared, we also need to regenerate the \nodeps{s} list for the nodes $s\in\canonpar{I}$ to account for the intervals that are no longer present in the subtree.
...
\Procedure{ClearInterval}{$s$}
	...
	\State Clear \nodecs{s}.
	\State Clear \nodeps{s}.
	\If{\leftc{s} and \rightc{s} defined}
...
Each node $s$ of the innermost segment tree represents a $D$-dimensional hyperrectangle \rect{s}, whose bounds are defined by the position of the node on each of the $D$ tree levels.
...
By induction the inner tree operations are done in time $O(\log^{D-1}\segsize)$, so the total time complexity is $O(\log^D\segsize)$.
\end{proof}
...
In each canonical parent $s\in\canonpari{R_d}{d}$ we split the contents of the subtree along axis $d$ before proceeding, and recompute the descendant information after finishing clearing the subtree.
...
\Procedure{ClearFromTree}{$d,n$}
	...
	\EndIf
	\State $\cleardt(d-1,n)$.
	\If{\leftu{s}{d} and \rightu{s}{d} defined}
...
Figure~\ref{fig:inter} displays an example of an intersection graph.
...
Figure~\ref{fig:light2d} displays the illumination process.
...
For any constant $D\ge 2$, the presented algorithm works in time $O(n^{D-1}\log^{D-1}n)$.
...
By Lemma~\ref{lem:segurmtime}, each clear operation takes time $O(n^{D-2}\log n + k)$ where $k$ is the number of cleared canonical nodes.
...
We gave a simple algorithm for the problem on a plane with running time $O(n\log n)$~\cite{revisited}.
...
These algorithms provide significant improvements over the previous best known results $O(n^{2.5}\log n)$~\cite{wagner} and $O(n^D\log n)$~\cite{de1992}.
