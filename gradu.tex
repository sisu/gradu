...
%Periaatetta kutsutaan usein \textit{vaiheittaiseksi valaistukseksi} (staged illumination), koska sitä voi havainnollistaa valaisun kautta:
%Asetetaan valonlähde haettavan polun alkupisteeseen.
%Kaikki tason pisteet, joihin valo koskee, ovat etäisyydellä~1.

%Kyseessä on klassinen ongelma, jota on tutkittu laajasti.
%Työssä esitellään uusia algoritmeja ongelman eri variaatioihin.
%Algoritmit ovat kirjoittajan omaa kontribuutiota, kehitetty yhteistyössä muiden tutkijoiden kanssa, ja ne on aiemmin julkaistu vertaisarvioiduissa lehdissä.
...
\subsection{Yleiskuva algoritmeista}
...
\begin{algorithmic}\label{seghaku}
...
Lemman~\ref{leikkaus1} ja algoritmin~\ref{seghaku} avulla on helppo toteuttaa rakenne, joka tukee välien tallennusta, ja mahdollistaa tiettyä pistettä leikkaavien välien etsimisen.
...
\subsection{Tason jakoalgoritmi}

Rektilineaarinen alue voidaan jakaa suorakaiteisiin jatkamalla jokaista vaakasuoraa janaa molempiin suuntiin kunnes ne törmäävät esteeseen.
Näin muodostunutta rakennetta kutsutaan alueen \emph{vaakasuoraksi hajotelmaksi}.
...
Vaakasuora hajotelma voidaan muodostaa pyyhkäisyviiva-algoritmilla.
...
\begin{algorithmic}\label{jako2d}
...
Algoritmin~\ref{jako2d} perusteella voimme todeta seuraavan.
...
\begin{proof}
Algoritmi~\ref{jako2d} järjestää esteiden vaakasuorat janat y-koordinaatin mukaan ajassa $O(n\log n)$, ja käy ne sitten läpi $O(n)$ askeleessa.
...
\section{Rajoitetun suuntaiset minimilinkkipolut}

\subsection{Rektilineaariset minimilinkkipolut}
