...
\begin{figure}\centering
	...
	\caption{There are many variants of the path finding problem in the plane producing different kind of paths.}\label{fig:paths}
\end{figure}
...
\begin{lem}Algorithm~\ref{alg:segurm} has running time $O(\segsize^{D-1}\log\segsize + k)$, where $k$ is the number of cleared canonical nodes.\end{lem}
\begin{proof}
...
For the purpose of deriving an upper bound, we assume that the obstacle faces are slightly perturbed such that each obstacle face has unique $z$-coordinate.
...
\begin{enumerate}
\item During a sweep in direction $+z$ we hit obstacle $o$.
...
The number of events on the plane of a single obstacle $o$ is $O(n)$, and the total number of events is thus $O(n^2)$.
\end{proof}
