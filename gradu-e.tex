...
\begin{algorithmic}
\State $D\gets$ decomposition of \fspace into cells using Algorithm~\ref{alg:split3d}.
...
\While{$E_d$ is not empty for some $d$}
	...
	\EndFor
	\ForAll{$d\in\set{\pm x,\pm y,\pm z}$}
		\State Illuminate in direction $d$ using Algorithm~\ref{alg:sweep3d}, generating events into $E_e'$ for $e\neq\pm d$.
	\EndFor
	\ForAll{$d\in\set{\pm x,\pm y,\pm z}$}
		\State $E_d\gets E_d'$.
	\EndFor
...
\begin{enumerate}
\item Remove all the pairs of \addEs with the position but opposite direction.
\item Merge all sets of aligned displays with the same $z$-range into a single larger display.
\end{enumerate}

The operations allow us to bound the number of \addEs generated during the illumination.
...
Since the number of \addEs is $O(n^2)$ by Lemma~\ref{lem:adde3}, the total time used in processing them is $O(n^2\log^2 n)$.
...
A natural follow up question is, whether these results could be improved further.
The 2D case was shown to be equivalently difficult as sorting the input, so the result is asymptotically optimal.
For the higher dimensional results no such upper bound is known.
