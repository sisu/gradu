...
\Procedure{ClearSubtree}{$n$}
	...
	\If{\nodeps{n} is not empty}
		\State Clear \nodeps{n}.
		\State \clearst(\leftc{n}).
		\State \clearst(\rightc{n}).
	\EndIf
\EndProcedure
\end{algorithmic}
\end{alg}
...
We perform the three steps by making two recursive traversals in the tree.
On the first traversal we push down all the hyperrectangles stored in \canonpar{R} and clear all the subtrees rooted at \canon{R}.
On the second tree traversal we recompute the information about the subtree to each node in \canonpar{R}.

\begin{alg}
Clear rectangle $R$ from $D$-dimensional unified segment tree.
\begin{algorithmic}
\State \cleardt($D,\texttt{root}$).
\State \recomp($D,\texttt{root}$).
\end{algorithmic}
\end{alg}

The procedures \cleardt and \recomp are defined below.

\begin{algorithmic}
\Procedure{ClearFromTree}{$d,n$}
	\If{$d=0$}
		\State \clearst($D,n,\maskof{R,n}$)
	\ElsIf{$\inter{n}\subseteq R_d$}
		\State $\cleardt(d-1,n)$.
	\ElsIf{$\rect{n}_d\cap R_d\neq\emptyset$}
		\State $\cleardt(d-1,n)$.
...
\Procedure{ClearSubtree}{$d,n,m$}
	\Comment Fully clears the subtree rooted at $n$.
	\If{$\neg\hasbits{n}{0}$}
		\State Return.
	\EndIf
	\While{$d>0$ and $m$ does not have bit $d$ set}
		\State $d\gets d-1$.
	\EndWhile
	\If{$d=0$}
		\ForAll{$b \in 0\dots 2^D-1$}
			\If{$b|m = 2^D-1$}
				\State $\hasbits{n}{b}\gets\texttt{false}.$
			\EndIf
		\EndFor
	\Else
		\State $\clearst(d-1,n,m)$.
		\State $\clearst(d,\leftu{n}{d},m)$.
		\State $\clearst(d,\rightu{n}{d},m)$.
	\EndIf
\EndProcedure

\Procedure{RecomputeSubtreeData}{$d,n$}
	\If{$d=0$}
		\State Reset all bits of $\hasbits{n}{}$ to $\texttt{false}$.
		\State $m\gets\maskof{R,n}$.
		\ForAll{$a \in 1\dots D$}
			\State $c_1\gets\leftu{n}{a}$.
			\State $2_1\gets\rightu{n}{a}$.
			\ForAll{$b \in 0 \dots 2^D-1$}
				\If{$b$ does not have bit $a$ set and $b|m\neq 2^N$}
